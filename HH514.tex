...
\subsection{General properties of the star/disk/outflow system of 170-337/HH~514}
...
but the flows must be launched from different radii.
...
Assuming a jet half opening angle of \ang{5} implies a mean jet mass loss rate
of \num{0.05} times that of the proplyd,
yielding \SI{4e-8}{\msun.yr^{-1}} albeit with a large uncertainty.

Proper motion measurements of the jet knot yield a
plane-of-sky tangential velocity of \SI{45 \pm 6}{km.s^{-1}}
\citep{odellyhenney08}
after rescaling to our adopted distance of \SI{410}{pc},
with a position angle that is consistent with the displacement
of the knot from 177-341 (almost due north).
Line-of-sight radial velocities can be determined
from our spectroscopic measurements and are found to be
approximately the same for the jet base and the knot,
with no variation between high and low ionization lines,
yielding \SI{+150 \pm 5}{km.s^{-1}}.
Combining these two values gives a jet inclination of \ang{-73 \pm 2}
...
then this result implies that the disk must be close to face-on.
In the disk literature, a different convention is used
in which the inclination \(i\) is between the rotation axis and the line of sight,
meaning that our jet results imply \(i = \ang{17}\) for 170-337.
...
implying \(i \approx \ang{40}\),
but this cannot be due to the disk inclination if the jet results are taken
at face value.
\citet{Boyden:2020a} derive a very low stellar mass (\(< \SI{0.1}{\msun}\))
from kinematic disk modelling,
but rescaling to our more face-on inclination would give \SI{0.3}{\msun},
which is fully compatible with the mass derived from stellar evolution tracks (see above).
However, a more serious conflict with the CO kinematics concerns the
sign and orientation of the velocity gradient,
which is north-south with blueshifted CO on the north side.
...
In the second case, one would expect redshifted instead of blueshifted CO on the north side.
...
It is well known that planetary formation process is capable of build up large grains in protoplanetary disks \citep[][]{Beckwith2000}.
...
\begin{figure*}
  \begin{minipage}{7.5cm}
    \centering\includegraphics[height=4cm,width=\columnwidth]{HH514I}
    \centerline{(a) Cut~1, HH~514 jet base.}
  \end{minipage}
  \begin{minipage}{7.5cm}
     \centering\includegraphics[height=4cm,width=\columnwidth]{HH514II}
    \centerline{(b) Cut~2, HH~514 knot.}
  \end{minipage}
 
  \begin{minipage}{7.5cm}
   \centering\includegraphics[height=4cm,width=\columnwidth]{neb_cut2}
   \centerline{(c) Cut~2, nebular component.}
  \end{minipage}
  \begin{minipage}{7.5cm}
    \centering\includegraphics[height=4cm,width=\columnwidth]{neb_cut3}
